...
`
	...
	// Verify client_hashes are SHA256 hashes (base64 encoded, 44 chars)
	if len(vqdResult.ClientHashes) > 0 {
		for i, hash := range vqdResult.ClientHashes {
			if len(hash) != 44 {
				t.Errorf("Expected client_hashes[%d] to be base64 SHA256 (44 chars), got %d chars: %s", i, len(hash), hash)
			}
		}
	}
